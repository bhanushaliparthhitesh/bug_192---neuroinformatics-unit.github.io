...
# ---------------------------------------------------------------------------
# Overrides Sphinx's default toctree-based Next/Prev buttons on a per-page
# basis via YAML front-matter in any .md file.
#
# Supported keys (docnames are relative to docs/source/, no extension):
#   next_page / prev_page             – target docname
#   next_page_title / prev_page_title – optional button label
#   Set next_page: "" to hide the button entirely.
#
# Current navigation chain (controlled via front-matter in each .md file):
#   index  ──Next──▶  people  ──Next──▶  projects  ──Next──▶  publications …
#
# To change a page's button, add front-matter to its .md file, e.g.:
#   ---
#   next_page: people          # docname of target (no extension)
#   next_page_title: "People"  # optional label; defaults to the page heading
#   prev_page: index
...
def _override_next_prev_page(app, pagename, templatename, context, doctree):
    """Override next/prev nav buttons using per-page front-matter metadata."""
    env = app.builder.env
    # YAML front-matter from the current page's .md file, or {} if absent.
    metadata = env.metadata.get(pagename, {})

    # Override the "Next ▶" button if next_page is set in front-matter.
    if "next_page" in metadata:
        next_docname = metadata["next_page"]
        if next_docname == "":
            context["next"] = None  # hide the button
        else:
    ...
    # Override the "◀ Previous" button if prev_page is set in front-matter.
    if "prev_page" in metadata:
        prev_docname = metadata["prev_page"]
        if prev_docname == "":
            context["prev"] = None  # hide the button
        else:
